...
public class EInfo extends JPanel  {
	...
	/**
	 * Nombre de tours
	 * Nombre de tuiles immergées/Nombre de tuiles total : % nombre de tuiles non-inondée
	 * Nombre de tuiles restantes/Nombre de tuiles total : % nombre de tuiles restantes
	 * Message impolabel_tuiles_inondeesrtant : Heliport inondé, joueur bloqué, tresor avec une case restante
	 */
	...
	public void update(HashMap<Utils.Tresor, Boolean> tresors, int nbTours, int tuiles_non_inondees, int tuiles_restantes, String message_alerte) {
		...
		for(int i = 0; i < this.panel_tresor.length; i++) {
			try {
				...
				if(!tresors.get(INDEX_TRESOR[i]).booleanValue()) ImageGrisee(this.image_tresor[i]);
			}
		}
